...
class VoiceSessionControllerTest {
    ...
    class StartSession {
        ...
        void startSession_created() throws Exception {
            ...
            result.andDo(print())
                    ...
                    .andExpect(header().string(HttpHeaders.LOCATION, "/api/v1/workspaces/1/voice/" + SESSION_ID))
    ...
    private void setId(VoiceSession session, Long id) {
        if (tryInvoke(session, "setId", id)) return;
        if (tryInvoke(session, "setVoiceSessionId", id)) return;
    ...
    private void safeSetEndedAt(VoiceSession session, LocalDateTime value) {
        if (tryInvoke(session, "setEndedAt", value)) return;
