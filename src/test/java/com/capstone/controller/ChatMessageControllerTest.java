...
class ChatMessageControllerTest {
  ...
  void sendMessage_shouldReturnCreatedMessage() throws Exception {
    ...
    when(chatMessageService.saveMessage(1L, userId, "안녕하세요!")).thenReturn(mockMessage);

    String requestBody = "{\"workspaceId\": 1, \"userId\": 123, \"content\": \"안녕하세요!\"}";
  ...
  void createFileMessage_shouldReturnResponse() throws Exception {
    ...
    // When & Then
    mockMvc.perform(post("/v1/chat/messages/file")
            .contentType(MediaType.APPLICATION_JSON)
            .content(body)
            .with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.csrf()))
  ...
  void sendMessage_withEmptyContent_shouldReturnBadRequest() throws Exception {
    // Given
    String requestBody = "{\"workspaceId\": 1, \"userId\": 123, \"content\": \"\"}";
  ...
  void sendMessage_withNullContent_shouldReturnBadRequest() throws Exception {
    // Given
    String requestBody = "{\"workspaceId\": 1, \"userId\": 123, \"content\": null}";
  ...
  void sendMessage_withNullWorkspaceId_shouldReturnBadRequest() throws Exception {
    // Given
    String requestBody = "{\"workspaceId\": null, \"userId\": 123, \"content\": \"안녕하세요!\"}";
  ...
  void getMessagesByWorkspace_shouldReturnMessageList() throws Exception {
    // Given
    User user1 = createMockUser(123L);
    User user2 = createMockUser(456L);
    ...
    // When & Then
    mockMvc.perform(get("/v1/chat/messages/workspace/1")
            .with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.csrf()))
        .andExpect(status().isOk())
  ...
  void getRecentMessages_shouldReturnLimitedMessages() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/v1/chat/messages/workspace/1/recent?limit=1")
            .with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
  ...
  void getMessagesByUser_shouldReturnUserMessages() throws Exception {
    ...
    List<ChatMessage> mockMessages = Arrays.asList(message1);
    when(chatMessageService.getMessagesByUser(userId)).thenReturn(mockMessages);

    // When & Then
    mockMvc.perform(get("/v1/chat/messages/user/123")
            .with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
  ...
  void getMessageCount_shouldReturnCount() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/v1/chat/messages/workspace/1/count")
            .with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.workspaceId").value(1))
