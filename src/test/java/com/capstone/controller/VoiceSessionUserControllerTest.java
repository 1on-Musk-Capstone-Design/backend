...
public class VoiceSessionUserControllerTest {
    ...
    public class JoinSession {
        ...
        void joinSession_created() throws Exception {
            ...
            when(voiceSessionUserService.joinSession(WORKSPACE_ID, SESSION_ID, USER_ID)).thenReturn(joined);
    ...
    public class LeaveSession {
        ...
        void leaveSession_ok() throws Exception {
            ...
            left.leave();
            when(voiceSessionUserService.leaveSession(WORKSPACE_ID, SESSION_ID, USER_ID)).thenReturn(left);

            mockMvc.perform(delete("/v1/workspaces/{workspaceId}/voice/{sessionId}/users/{userId}", WORKSPACE_ID, SESSION_ID, USER_ID)
                    .with(csrf()))
                    .andDo(print())
   ...
   public class MoveToSession {
        ...
        void moveToSession_ok() throws Exception {
            ...
            when(voiceSessionUserService.moveToSession(eq(WORKSPACE_ID), eq(SESSION_ID), eq(toSessionId), eq(USER_ID))).thenReturn(moved);

            String requestBody = "{\"workspaceUserId\": " + USER_ID + "}";

            mockMvc.perform(post("/v1/workspaces/{workspaceId}/voice/{sessionId}/users/move", WORKSPACE_ID, SESSION_ID)
    ...
    public class GetActiveUsers {
        ...
        void getActiveUsers_ok() throws Exception {
            ...
            when(voiceSessionUserService.getActiveUsers(WORKSPACE_ID, SESSION_ID)).thenReturn(activeUsers);

            mockMvc.perform(get("/v1/workspaces/{workspaceId}/voice/{sessionId}/users", WORKSPACE_ID, SESSION_ID))
                    .andDo(print())
                    .andExpect(status().isOk())
    ...
    public class CountActiveUsers {
        ...
        void countActiveUsers_ok() throws Exception {
            ...
            mockMvc.perform(get("/v1/workspaces/{workspaceId}/voice/{sessionId}/users/count", WORKSPACE_ID, SESSION_ID))
                    .andDo(print())
                    .andExpect(status().isOk())
    ...
    private VoiceSessionUser createVoiceSessionUser(Long sessionId, Long workspaceUserId, String userName, Long voiceSessionUserId) {
        // 세션
