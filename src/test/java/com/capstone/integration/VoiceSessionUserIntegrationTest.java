...
class VoiceSessionUserIntegrationTest {
    ...
    void findActiveUsers_shouldReturnOnlyActive() {
        ...
        List<VoiceSessionUser> activeUsers = voiceSessionUserRepository.findBySessionIdAndLeftAtIsNull(session.getId());
    ...
    void moveSession_shouldUpdateLeftAtAndCreateNewSessionUser() {
        ...
        List<VoiceSessionUser> oldSessionActiveUsers = voiceSessionUserRepository.findBySessionIdAndLeftAtIsNull(oldSession.getId());
        List<VoiceSessionUser> newSessionActiveUsers = voiceSessionUserRepository.findBySessionIdAndLeftAtIsNull(newSession.getId());

        assertThat(oldSessionActiveUsers).isEmpty();
        assertThat(newSessionActiveUsers).hasSize(1);
        assertThat(newSessionActiveUsers.get(0).getWorkspaceUser().getId()).isEqualTo(workspaceUser.getId());
    }
}