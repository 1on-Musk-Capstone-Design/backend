...
public class ChatMessageController {
  ...
  /**
   * 파일/이미지 메시지(JSON 메타) 생성 POST /v1/chat/messages/file
   */
  @PostMapping("/file")
  public ResponseEntity<ChatMessageDtos.Response> createFileMessage(
      @RequestBody ChatMessageDtos.FileMessageRequest request) {
    ...
    if (messageType == null || (!"image".equals(messageType) && !"file".equals(messageType))) {
      messageType =
          request.getMimeType() != null && request.getMimeType().startsWith("image/") ? "image"
              : "file";
    }
  ...
  /**
   * 채팅 메시지 전송 POST /v1/chat/messages
   */
  @PostMapping
  public ResponseEntity<ChatMessageDtos.Response> sendMessage(
      @RequestBody ChatMessageDtos.SendRequest request) {
  ...
  /**
   * 특정 워크스페이스의 채팅 메시지 목록 조회 GET /v1/chat/messages/workspace/{workspaceId}
   */
  @GetMapping("/workspace/{workspaceId}")
  public ResponseEntity<List<ChatMessageDtos.Response>> getMessagesByWorkspace(
  ...
  /**
   * 특정 워크스페이스의 최근 채팅 메시지 조회 GET /v1/chat/messages/workspace/{workspaceId}/recent?limit=10
   */
  ...
  /**
   * 특정 사용자의 채팅 메시지 목록 조회 GET /v1/chat/messages/user/{userId}
   */
  ...
  /**
   * 특정 워크스페이스의 채팅 메시지 개수 조회 GET /v1/chat/messages/workspace/{workspaceId}/count
   */
  @GetMapping("/workspace/{workspaceId}/count")
  public ResponseEntity<ChatMessageDtos.CountResponse> getMessageCount(
      @PathVariable Long workspaceId) {
